...
void handle_sysevt(VMINT message, VMINT param) {
    ...
    switch (message){
		...
		// CRAIG turn RGB LED to solid BLUE to indicate we got here

#define VM_PIN_RGB_B VM_PIN_P12
#define VM_PIN_RGB_G VM_PIN_P15
#define VM_PIN_RGB_R VM_PIN_P17

//		vm_dcl_config_pin_mode(VM_PIN_RGB_B, VM_DCL_PIN_MODE_GPIO);
//		vm_dcl_config_pin_mode(VM_PIN_RGB_G, VM_DCL_PIN_MODE_GPIO);
//		vm_dcl_config_pin_mode(VM_PIN_RGB_R, VM_DCL_PIN_MODE_GPIO);

		gpio_handle = vm_dcl_open(VM_DCL_GPIO, VM_PIN_RGB_R);
		vm_dcl_control(gpio_handle, VM_DCL_GPIO_COMMAND_SET_DIRECTION_OUT, 0);
		vm_dcl_control(gpio_handle, VM_DCL_GPIO_COMMAND_WRITE_HIGH, (void *)0);
		vm_dcl_close(gpio_handle);

		gpio_handle = vm_dcl_open(VM_DCL_GPIO, VM_PIN_RGB_B);
		vm_dcl_control(gpio_handle, VM_DCL_GPIO_COMMAND_SET_DIRECTION_OUT, 0);
		vm_dcl_control(gpio_handle, VM_DCL_GPIO_COMMAND_WRITE_LOW, (void *)0);
		vm_dcl_close(gpio_handle);

		gpio_handle = vm_dcl_open(VM_DCL_GPIO, VM_PIN_RGB_G);
		vm_dcl_control(gpio_handle, VM_DCL_GPIO_COMMAND_SET_DIRECTION_OUT, 0);
		vm_dcl_control(gpio_handle, VM_DCL_GPIO_COMMAND_WRITE_HIGH, (void *)0);
		vm_dcl_close(gpio_handle);
...
#elif defined(__HDK_LINKIT_REPHONE__)
